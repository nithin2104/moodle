...
use repository_coursebank\browser\coursebank_browser;
...
class helper {
     ...
     * @return \repository_coursebank\browser\coursebank_browser|null The content bank repository browser
     */
    public static function get_coursebank_browser(\context $context): ?coursebank_browser {
        switch ($context->contextlevel) {
            case CONTEXT_SYSTEM:
                return new \repository_coursebank\browser\coursebank_browser_context_system($context);
            case CONTEXT_COURSECAT:
                return new \repository_coursebank\browser\coursebank_browser_context_coursecat($context);
            case CONTEXT_COURSE:
                return new \repository_coursebank\browser\coursebank_browser_context_course($context);
        }
        return null;
    }
     ...
     *
     * @param mixed $content The content bank content
     * @return array|null The content bank content node
     */
    public static function create_coursebank_content_node($content): ?array {
        global $OUTPUT;
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($content->contextid, 'local_coursebank', 'coursebank', $content->id);
        if ($files) {
            foreach ($files as $f) {
                $file = $f;
            }
