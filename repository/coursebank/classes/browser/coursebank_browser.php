...
namespace repository_coursebank\browser;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class coursebank_browser {
    ...
    public function get_navigation(): array {
        ...
        $currentnavigationnode = \repository_coursebank\helper::create_navigation_node($this->context);
        ...
        while ($parent !== null) {
            $parentnavigationnode = \repository_coursebank\helper::create_navigation_node($parent->context);
     ...
     *
     * @return coursebank_browser|null The content bank browser of the parent context
     */
    private function get_parent(): ?self {
        if ($parentcontext = $this->context->get_parent_context()) {
            return \repository_coursebank\helper::get_coursebank_browser($parentcontext);
        }
        return null;
    }
    ...
    private function get_context_folders(): array {
        ...
        return array_reduce($children, function ($list, $child) {
            $browser = \repository_coursebank\helper::get_coursebank_browser($child);
            if ($browser->can_access_content()) {
                $name = $child->get_context_name(false);
                $path = base64_encode(json_encode(['contextid' => $child->id]));
                $list[] = \repository_coursebank\helper::create_context_folder_node($name, $path);
            }
            return $list;
        }, []);
    }
    ...
    private function get_contentbank_content(): array {
        global $DB;
        $contents = array_values($DB->get_records('local_coursebank', ['contextid' => $this->context->id]));
