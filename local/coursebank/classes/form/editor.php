 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class editor extends moodleform {
    ...
    private $content;
    /** @var $context The content being edited */
    private $context;
    ...
    protected function definition() {
        ...
        if ($id) {
            ...
            $record = $DB->get_record('local_coursebank', ['id' => $id]);
            $this->content = $record;
            $fs = get_file_storage();
            $files = $fs->get_area_files($record->contextid, 'local_coursebank', 'coursebank', $record->id);
            if ($files) {
                foreach ($files as $f) {
                    $file = $f;
                }
            ...
            $this->h5peditor->set_library($library, $this->_customdata['contextid'], 'local_coursebank', 'coursebank');
        }
    ...
    public function save_content(stdClass $data): int {
        ...
        if (!isset($data->h5pid)) {
            ...
            $record->contenttype = 'h5p';
            $record->contextid = $data->contextid;
            $record->usercreated = $USER->id;
            $record->timecreated = time();
            $record->timemodified = time();
            $newcontent = $DB->insert_record('local_coursebank', $record);
            if ($file && $newcontent) {
                $updatedfilerecord = new stdClass();
                $updatedfilerecord->id = $file->get_id();
                $updatedfilerecord->itemid = $newcontent;
            ...
            $record->name = $file->get_filename();
            $record->timemodified = time();
            $DB->update_record('local_coursebank', $record);
            $contentid = $record->id;
        }
