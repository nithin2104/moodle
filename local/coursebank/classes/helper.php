...
class helper {
    ...
    /**
     * Getting course bank page ready for the breadcrumbs.
     *
    ...
    public static function get_page_ready(\context $context, string $title, bool $internal = false): void {
        ...
        $cburl = new \moodle_url('/local/coursebank/index.php', ['contextid' => $context->id]);
        ...
        switch ($context->contextlevel) {
            case CONTEXT_COURSE:
                ...
                \navigation_node::override_active_url(new \moodle_url('/local/course/view.php', ['id' => $courseid]));
                $PAGE->navbar->add($title, $cburl);
                $PAGE->set_pagelayout('incourse');
                break;
            case CONTEXT_COURSECAT:
                $coursecat = $context->instanceid;
                \navigation_node::override_active_url(new \moodle_url('/local/course/index.php', ['categoryid' => $coursecat]));
                $PAGE->navbar->add($title, $cburl);
                $PAGE->set_pagelayout('coursecategory');
                break;
            default:
                if ($node = $PAGE->navigation->find('coursebank', \global_navigation::TYPE_CUSTOM)) {
                    $node->make_active();
                }
